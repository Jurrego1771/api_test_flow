...
test.describe("Schedule-Job API (Live Stream)", () => {
  ...
  test.afterAll(async ({ authRequest }) => {
    for (const { liveId, jobId } of [...createdJobIds]) {
      try {
        await authRequest.delete(`${SCH_BASE(liveId)}/${jobId}`);
  ...
  test.describe("1. GET — Consulta de Schedule-Job", () => {
    ...
    test("TC_SCH_001_GET_ScheduleJobFromExistingLive", async ({ authRequest }) => {
    ...
    test("TC_SCH_002_GET_ScheduleJob_NotFound_FakeId", async ({ authRequest }) => {
      const fakeId = "000000000000000000000000";
      const res = await authRequest.get(SCH_BASE(fakeId));
    ...
    test("TC_SCH_003_GET_ScheduleJob_ResponseHeaders", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_004_GET_ScheduleJob_DataStructureIfPresent", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
  ...
  test.describe("2. POST — Crear y Actualizar Schedule-Job", () => {
    ...
    test("TC_SCH_005_POST_CreateScheduleJobBasic", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_007_POST_CreateScheduleJob_MissingRequiredFields", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_009_POST_UpdateScheduleJob_ChangeHour", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_010_POST_CreateScheduleJob_PastDate_Behavior", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
  ...
  test.describe("3. DELETE — Eliminar Schedule-Job", () => {
    ...
    test("TC_SCH_011_DELETE_ScheduleJobById", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
  ...
  test.describe("4. Validaciones de Fechas y Tiempos", () => {
    ...
    test("TC_SCH_015_POST_EndHourBeforeStartHour_OnSameDay", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_016_POST_InvalidMinuteValue_ShouldReject", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_017_POST_SameonDateAndoffDate_ShouldReject", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_018_POST_RecurringWithoutDays_Behavior", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_019_POST_RecurringWithAllDays", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
  ...
  test.describe("6. Edge Cases", () => {
    ...
    test("TC_SCH_024_POST_EmptyPayload_ShouldRejectOrAccept", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_025_POST_ExtraUnknownFields_ShouldNotCrash", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_026_GET_MalformedLiveId_ShouldReturn400or404", async ({ authRequest }) => {
      const malformedId = "!@#$%^&*invalid-id";
      const res = await authRequest.get(SCH_BASE(malformedId));

      expect([400, 404, 422]).toContain(res.status());
    });

    test("TC_SCH_027_GET_JobById_WhenJobDoesNotExist", async ({ authRequest }) => {
      const fakeJobId = "000000000000000000000099";
      const res = await authRequest.get(`${SCH_BASE(LIVE_ID)}/${fakeJobId}`);
    ...
    test("TC_SCH_028_POST_VeryLongStringInDates_ShouldReject", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_029_GET_ResponseTime_ShouldBeUnder5s", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
    ...
    test("TC_SCH_030_POST_NullValues_ShouldRejectOrHandle", async ({ authRequest }) => {
      if (!(await ensureScheduleApiAvailable(authRequest))) return;
